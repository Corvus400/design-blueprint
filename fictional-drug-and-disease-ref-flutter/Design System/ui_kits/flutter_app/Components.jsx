...
const Chip = ({ axis, value, label }) => {
  const palette = isDarkMode() ? CHIP_FG_DARK : CHIP_FG;
  const fg = palette[axis]?.[value] || (isDarkMode() ? '#C5CAD0' : '#3F4347');
...
const SegControl = ({ value, onChange, options }) => (
  <div style={{
    display: 'flex', background: '#EFEFF4', borderRadius: 9,
    padding: 2, width: '100%',
  }}>
    {options.map((o) => {
...
const SearchField = ({ value, onChange, hint, onClear, onSubmit }) => (
  <div style={{
    flex: 1, height: 'var(--field-h-phone)', background: '#EBEBEF', borderRadius: 12,
    ...
    <input value={value} placeholder={hint}
      ...
      style={{
        ...
        font: '400 15px/1.4 var(--font-jp)', color: '#1A1C1E', minWidth: 0,
      }}
    />
    {value && (
...
const SearchTopChrome = ({ tab, onTab, query, onQuery, onSubmit, onClear }) => (
  <div style={{
    ...
    borderBottom: '0.5px solid rgba(60,60,67,.13)',
    display: 'flex', flexDirection: 'column', flex: '0 0 auto',
  }}>
    <div style={{ font: '700 var(--search-title-fs)/1.2 var(--font-jp)', color: '#1A1C1E' }}>検索</div>
...
const DrugCard = ({ item, onTap }) => (
  <div onClick={onTap} style={{
    background: '#fff', border: '0.5px solid rgba(60,60,67,.13)',
    borderRadius: 10, padding: 12, marginTop: 8,
    display: 'flex', gap: 12, alignItems: 'center', cursor: 'pointer',
  }}>
    <div style={{
      width: 56, height: 84, borderRadius: 8, overflow: 'hidden', flex: '0 0 auto',
      background: '#EFEFF4', border: '0.5px solid rgba(60,60,67,.13)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      <span className="material-symbols-outlined" style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
      ...
      <div style={{
        marginTop: 4, font: '700 14px/1.3 var(--font-jp)', color: '#1A1C1E',
...
const AppliedChipRail = ({ chips, onRemoveAt }) => {
  ...
  return (
    <div style={{
      ...
      borderTop: '0.5px solid rgba(60,60,67,.13)',
...
const BottomSheet = ({ open, title, onClose, children }) => (
  <>
    ...
    <div style={{
      ...
      background: '#fff', borderRadius: '20px 20px 0 0', paddingBottom: 20,
      ...
      <div style={{
        ...
        borderBottom: '0.5px solid rgba(60,60,67,.13)',
        font: '700 16px/1.2 var(--font-jp)', color: '#1A1C1E',
      }}>{title}</div>
      {children}
    </div>
  </>
  ...
  return (
    <>
      <div style={{
        ...
        <span style={{ font: '500 12.5px/1 var(--font-jp)', color: '#1A1C1E' }}>
          合計 {count} 件
        </span>
...
const FilterSheet = ({ open, onClose, tab, applied, onClearAll, onAddSample }) => {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title={tab === 'drugs' ? '医薬品の絞り込み' : '疾患の絞り込み'}>
            ...
            <div key={g.id} style={{ borderBottom: last ? 'none' : '0.5px solid rgba(60,60,67,.10)' }}>
              <button onClick={() => setExpanded(on ? null : g.id)} style={{
                ...
                <span style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <span style={{ font: '700 14px/1.2 var(--font-jp)', color: '#1A1C1E' }}>{g.t}</span>
      ...
      <div style={{
        padding: '12px 16px 0', borderTop: '0.5px solid rgba(60,60,67,.13)',
        display: 'flex', gap: 10,
      }}>
        <button onClick={onClearAll} style={{
          flex: 1, height: 44, border: '1px solid var(--d-outline-variant)', background: '#fff',
          borderRadius: 22, font: '600 14px/1 var(--font-jp)', color: '#1A1C1E', cursor: 'pointer',
        }}>リセット</button>
...
const BottomNav = ({ value, onChange }) => {
  ...
  return (
    <nav style={{
      display: 'flex', height: 80, background: '#fff',
      borderTop: '0.5px solid rgba(60,60,67,.13)', flex: '0 0 auto',
    }}>
      {dests.map((d) => {
...
const DetailFooter = ({ bookmarked, onToggleBookmark, onDoseCalc, showDoseCalc = true }) => (
  <div style={{
    background: '#fff', flex: '0 0 auto', height: 64,
...
Object.assign(window, {
  ...
  DRUG_FILTER_AXES, DISEASE_FILTER_AXES, CHIP_FG_DARK, labelFor,
