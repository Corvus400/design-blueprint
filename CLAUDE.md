...
- For broad recurrence-prevention requests, run `npm run session:conflict-audit -- --repo . --inventory` before proposing permanent fixes. Use `cwd_exact` as the authoritative set, use remote/project/page alias hits only as completeness candidates, and never paste raw transcript text into reports.
